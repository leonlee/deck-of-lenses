...
To improve the companion, edit `chapters.json`, preserve its source references, and regenerate the Markdown and site. Validation checks bilingual completeness, chapter coverage, source consistency, numbering, and chapter references. It also checks the canonical source index against a SHA-256 pinned in `scripts/book.py`, so CI rejects changed lens anchors even without the EPUB. JSON formatting and key order do not affect this digest; chapter note edits remain independent. Updating the pinned digest requires reviewing a fresh extraction against the original EPUB, rather than accepting an edited index. Optional EPUB validation additionally re-extracts all anchors from the actual source. Semantic accuracy and translation quality still require editorial review.
